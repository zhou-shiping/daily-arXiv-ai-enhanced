...
TL;DR: Data-driven framework for Kohn-Sham DFT that bypasses iterative wavefunction optimization by using pre-sampled atomic configurations to construct a reduced basis for efficient electronic structure calculations.


<details>
  <summary>Details</summary>
Motivation: To exploit redundancy in electronic structure calculations during molecular dynamics simulations and avoid the computational expense of iterative wavefunction optimization at each time step.

Method: Sample representative atomic configurations a priori, construct a low-dimensional basis that approximates the electronic structure subspace, then use this reduced basis in a direct solver for the electronic single particle density matrix.

Result: Demonstrated efficacy in Born-Oppenheimer molecular dynamics of a water molecule, accurately reproducing key structural properties (bond lengths and bond angle) compared to full first-principles molecular dynamics.

Conclusion: Data-driven approaches show potential for developing efficient electronic structure solvers for first-principles simulations by bypassing iterative wavefunction optimization.
...
TL;DR: Novel theoretical framework for Gaussian process regression error analysis using function-space decomposition, with weighted Jacobi iterative method using GP regression for parameter prediction and convergence analysis.


<details>
  <summary>Details</summary>
Motivation: To develop a more general theoretical framework for analyzing Gaussian process regression errors and to accelerate Jacobi iterative methods through intelligent parameter prediction.

Method: Function-space decomposition framework for GP regression error analysis, weighted Jacobi iterative method using GP regression for parameter prediction, and convergence analysis with compatible conditions.

Result: GP regression-predicted parameters significantly accelerate Jacobi iteration convergence speed in experiments.

Conclusion: The proposed framework enables general error analysis and the GP-based parameter prediction effectively accelerates Jacobi iterative convergence.
...
TL;DR: Structure-preserving DG methods for Cahn-Hilliard-Navier-Stokes with degenerate mobility, featuring enhanced coercivity-stability control and preservation of key physical properties.


<details>
  <summary>Details</summary>
Motivation: Need for robust numerical methods for Cahn-Hilliard-Navier-Stokes equations with degenerate mobility that preserve physical structure (mass conservation, energy dissipation, maximum principle) while maintaining computational efficiency.

Method: Develop SWIPD-L and SIPGD-L discontinuous Galerkin methods with parametrized mobility fluxes and edge-wise mobility treatments for enhanced coercivity-stability control. Prove coercivity for generalized trilinear form and demonstrate optimal convergence rates.

Result: Methods preserve mass conservation, energy dissipation, and discrete maximum principle. Show optimal convergence rates and similar stability compared to existing SIPG-L and SWIP-L methods. Validation on hp-adaptive meshes demonstrates significant computational savings without accuracy loss for both standalone Cahn-Hilliard and coupled systems.

Conclusion: The proposed structure-preserving DG methods provide efficient, accurate, and physically consistent numerical solutions for Cahn-Hilliard-Navier-Stokes equations with degenerate mobility, offering computational advantages through hp-adaptivity while maintaining key physical properties.
...
TL;DR: High-order isogeometric method for magnetoquasistatic eddy-current problems using Biot-Savart source fields and finite-element reaction fields, avoiding coil meshing and enabling high-order accuracy.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient high-order formulation for magnetoquasistatic eddy-current problems that avoids the need for coil meshing, supports arbitrary winding paths, and enables high-order field approximation within reduced computational domains.

Method: Decomposition into Biot-Savart-driven source fields and finite-element reaction fields, generalization of reduced magnetic vector potential framework to quasistatic regime, and consistent high-order spline discretization based on surface-only Biot-Savart evaluation.

Result: The method achieves optimal convergence rates and identifies practical requirements for high-order accuracy: geometric regularity of enclosing interface, accurate kernel quadrature, and compatible trace spaces for source-reaction coupling.

Conclusion: The proposed high-order isogeometric formulation successfully addresses magnetoquasistatic eddy-current problems with coil meshing avoidance, arbitrary winding support, and high-order accuracy when practical implementation requirements are met.
...
TL;DR: This paper addresses the problem of approximating non-reversible Markov chains with reversible ones while preserving sparsity, formulated as a matrix nearness optimization problem.


<details>
  <summary>Details</summary>
Motivation: Many applications produce non-reversible Markov chains, but reversibility is crucial for algorithms like Metropolis-Hastings and other MCMC methods. There's a need to approximate non-reversible chains with reversible ones with minimal modification.

Method: The authors formulate the task as a matrix nearness problem, focusing on sparse transition matrices. They solve this as a quadratic programming optimization problem.
...
Conclusion: The framework provides a principled method to enforce reversibility and sparsity in Markov chains, with applications in MCMC, computational chemistry, and data-driven modeling.
...
TL;DR: A hyperbolic PDE model for urban tram networks with stochastic passenger boarding, analyzed in measure-valued sense and extended with uncertainty modeling for delays/interruptions.


<details>
  <summary>Details</summary>
Motivation: To develop a mathematical framework for modeling urban tram networks that captures passenger transport dynamics and incorporates real-world uncertainties like delays and service interruptions.

Method: Hyperbolic partial differential equation for passenger transport along network, coupled with stochastic processes for passenger boarding. Solutions considered in measure-valued sense, extended with uncertainty modeling through numerical study.

Result: Framework developed and analyzed numerically with uncertainty extensions. Robustness assessed using appropriate risk measures.

Conclusion: The proposed modeling framework provides a mathematical foundation for analyzing urban tram networks under uncertainty, enabling robustness assessment through risk measures.
...
TL;DR: ECAV is an entropy-correcting artificial viscosity method that can be implemented with very small viscosity coefficients, analyzed here with LDG discretization, showing O(h) coefficient bounds and contact-preserving properties.


<details>
  <summary>Details</summary>
Motivation: To develop and analyze an entropy correction artificial viscosity (ECAV) method that enforces semi-discrete entropy inequality while maintaining computational efficiency and preserving contact discontinuities.

Method: Analyzes ECAV when discretized using local discontinuous Galerkin (LDG) method, proves mathematical bounds on viscosity coefficient, demonstrates contact-preserving properties, and compares with traditional shock capturing artificial viscosity methods.

Result: Proves O(h) upper bound on ECAV coefficient indicating no restrictive time-step condition, shows ECAV is contact preserving, and provides comparison with traditional artificial viscosity methods.

Conclusion: ECAV with LDG discretization provides an effective entropy-correcting artificial viscosity approach that maintains computational efficiency (no restrictive time-step conditions), preserves contact discontinuities, and offers advantages over traditional shock capturing methods.
...
TL;DR: The paper shows that particle paths in scalar conservation laws are geodesics in a metric on the diffeomorphism group, representing extremals of an action functional.


<details>
  <summary>Details</summary>
Motivation: To establish a geometric interpretation of particle paths in conservation laws, connecting them to geodesic flows on infinite-dimensional diffeomorphism groups and action principles.

Method: Uses Lagrangian formulation of scalar conservation laws, analyzes the diffeomorphism group representation, and applies variational principles to show particle paths are extremals of action functionals.

Result: Proves that for scalar conservation laws, particle paths correspond to geodesics in some metric on the diffeomorphism group, and extends this to systems like isentropic gas dynamics.

Conclusion: Provides a geometric framework connecting conservation laws to geodesic flows on diffeomorphism groups, offering new insights into the structure of these PDEs through variational principles.
...
TL;DR: Global well-posedness for compressible Euler system with damping in L^p-type critical Besov spaces for 1≤p<2 using new product estimates in hybrid Besov spaces.


<details>
  <summary>Details</summary>
Motivation: To establish global-in-time well-posedness for the Cauchy problem of compressible Euler system with damping in L^p-type critical Besov spaces, which extends previous results and handles the challenging case of 1≤p<2.

Method: Develops a new product estimate in L^2-L^p hybrid Besov spaces to handle the nonlinear terms in the compressible Euler system with damping, enabling analysis in the critical Besov space framework for the range 1≤p<2.

Result: Proves global-in-time well-posedness for the compressible Euler system with damping in L^p-type critical Besov spaces for 1≤p<2, extending previous results that were limited to p=2 or other ranges.

Conclusion: The paper successfully establishes global well-posedness for compressible Euler equations with damping in critical Besov spaces for 1≤p<2 through the development of new hybrid Besov space product estimates, advancing the mathematical theory of compressible fluid dynamics.
...
<details>
  <summary>Details</summary>
Motivation: To establish rigorous mathematical foundations for subsonic steady Euler flows past axisymmetric obstacles, extending previous results to accommodate flows with large vorticity under structural conditions.

Method: Combines strong maximum principle with refined continuity argument to establish non-degeneracy of axial velocity, uses uniform integral estimates for asymptotic behavior analysis.
...
Conclusion: Establishes rigorous mathematical framework for axisymmetric subsonic Euler flows, extending beyond two-dimensional case to handle large vorticity flows with proper structural conditions.
...
TL;DR: The paper studies viscous-dispersive shock profiles with infinite oscillations in the KdVB equation, establishing detailed shock structures and proving L² contraction under large perturbations with time-dependent shifts, leading to stability results and zero viscosity-dispersion limits.


<details>
  <summary>Details</summary>
Motivation: To understand the detailed structure and stability properties of viscous-dispersive shock profiles with infinite oscillations in the Korteweg-de Vries-Burgers equation, particularly focusing on how local extrema converge and establishing contraction properties under large perturbations.

Method: First establishes detailed structures of the shock wave including convergence rates of local extrema to the left end state. Then exploits these structural properties to prove L² contraction property of the shock profile under arbitrarily large perturbations, up to a time-dependent shift.

Result: Proves L² contraction property under arbitrarily large perturbations with time-dependent shift, which implies both time-asymptotic stability and uniform stability with respect to viscosity and dispersion coefficients. This uniformity yields zero viscosity-dispersion limits.

Conclusion: The detailed structural analysis of KdVB shock profiles enables proof of contraction properties that lead to strong stability results and the ability to take zero viscosity-dispersion limits, advancing understanding of oscillatory shock behavior in viscous-dispersive systems.
...
TL;DR: Existence of synchronized normalized solutions for two-component BEC system that concentrate on circles in ℝ² as a parameter tends to zero.


<details>
  <summary>Details</summary>
Motivation: To study normalized solutions (with L²-constraint) of two-component Bose-Einstein condensates systems, particularly focusing on high-dimensional concentration phenomena that were previously unexplored in such systems.

Method: Finite-dimensional reduction method combined with local Pohozaev identities to construct vector radial solutions that concentrate on circles.

Result: For any α>0, γ>0 and β in specified ranges, existence of synchronized solutions concentrating on high-dimensional subsets (circles) of ℝ² is established when (α+γ-2β)/(αγ-β²) tends to zero.

Conclusion: The paper fills the gap in understanding high-dimensional concentrated normalized solutions for two-component BEC systems, demonstrating concentration on circles as a specific parameter ratio approaches zero.
...
TL;DR: The paper establishes sharp pointwise decay estimates for solutions to semilinear wave equations satisfying the weak null condition, showing both upper and lower bounds, and applies these estimates to demonstrate energy blow-up and energy cascade phenomena.


<details>
  <summary>Details</summary>
Motivation: To understand the long-time behavior of solutions to semilinear wave equations that satisfy the weak null condition, which serve as simplified models for the Einstein vacuum equations in general relativity. The goal is to obtain precise decay estimates that can reveal interesting dynamical phenomena like energy blow-up and cascade.

Method: The authors study a system of semilinear wave equations under the weak null condition. They establish both lower and upper bounds for pointwise decay of small data solutions, showing that the difference between the solution and its leading-order term decays faster in the retarded time variable u = t - r. They prove these estimates are sharp for generic classes of small initial data with sufficient decay.

Result: 1) Sharp pointwise decay estimates showing both upper and lower bounds for solutions. 2) The difference between the solution and its leading-order term is dominated by lower-order terms decaying faster in retarded time. 3) For a generic class of small initial data, one component's energy has a lower bound that grows to infinity as t → +∞ (blowing up at infinity). 4) This component generically exhibits energy cascade from high to low frequencies.

Conclusion: The paper provides complete sharp pointwise decay estimates for semilinear wave equations under the weak null condition, revealing that despite the weak null condition preventing finite-time blow-up, interesting long-time phenomena like energy blow-up at infinity and energy cascade can still occur for generic initial data.
...
TL;DR: Persistence of boundary smoothness for vortex patches in quasi-geostrophic shallow-water equations, with convergence to Euler solutions as Rossby radius parameter ε → 0.


<details>
  <summary>Details</summary>
Motivation: The QGSW equations generalize Euler equations with an additional Rossby radius parameter ε⁻¹, modifying the relationship between streamfunction and vorticity. Understanding vortex patch boundary behavior and the connection to classical Euler equations is important for geophysical fluid dynamics.

Method: Mathematical analysis proving persistence of boundary smoothness for vortex patches in QGSW equations. Also establishes convergence of QGSW solutions to corresponding Euler solutions as ε → 0 in little Hölder spaces.

Result: Successfully proves that vortex patch boundaries remain smooth under QGSW dynamics. Shows local-in-time convergence of QGSW solutions to Euler solutions as the Rossby radius parameter ε approaches zero.

Conclusion: The QGSW equations preserve boundary regularity of vortex patches similarly to Euler equations, and converge to Euler dynamics in the limit of large Rossby radius (small ε), establishing a rigorous connection between these geophysical and classical fluid models.
...
<details>
  <summary>Details</summary>
Motivation: To demonstrate that all cases postulated in the soliton resolution theorem for finite time blow-up indeed occur, specifically showing that arbitrarily large numbers of concentrating concentric bubble profiles exist for the energy critical Wave Maps equation.

Method: Construct n-bubble solutions for the Wave Maps equation in the k=2 co-rotational setting, with bubbles concentrating at scales λ₁(t) ≫ λ₂(t) ≫ ... ≫ λₙ(t), where λₙ(t) = t⁻¹|log t|^β (β > 3/2), and λⱼ(t) ≳ exp(∫ₜᵗ⁰ λⱼ₊₁(s)ds) for j < n.

Result: The paper successfully constructs n-bubble solutions for any n ∈ ℕ, showing that arbitrarily large numbers of concentrating concentric bubbles with alternating signs exist, confirming the completeness of cases in the soliton resolution theorem for finite time blow-up.

Conclusion: The entirety of cases postulated in the soliton resolution theorem for finite time blow-up indeed occur for the energy critical Wave Maps equation, provided the concentric collapsing bubbles have alternating signs, demonstrating the rich structure of blow-up solutions.
...
TL;DR: The paper extends semiclassical approximations for coherent state evolution beyond the log(h) time barrier by incorporating anisotropic spreading along invariant submanifolds.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitation of squeezed coherent state approximations that break down at times ~|log h|/(6λ₀), and to provide valid approximations up to Ehrenfest time ~|log h|/(2λ₀) where wavepackets spread macroscopically.

Method: Combine WKB states in transverse hyperbolic directions with squeezed states along a normally hyperbolic invariant submanifold K, where dynamics is slow compared to transverse hyperbolic directions.

Result: Develops a representation of propagated states valid up to times |t| ≤ C|log h| with larger C, describing states as microlocalized on isotropic submanifolds rather than single points.

Conclusion: Propagated coherent states should be viewed as microlocalized on isotropic submanifolds corresponding to transverse unstable directions, extending beyond point-localized squeezed state approximations.
...
TL;DR: Proves convergence to shifted profiles for monostable waves under optimal topology perturbations, using resolvent kernel estimates to handle weakly localized perturbations and construct phase shifts even without Evans function zeros.


<details>
  <summary>Details</summary>
Motivation: To establish stability results for monostable reaction-diffusion waves under optimal conditions, addressing challenges with weakly localized perturbations and situations where traditional Evans function approaches fail due to translational eigenvalues not corresponding to zeros of the Evans function.

Method: Uses explicit resolvent kernel estimates to analyze stability, enabling handling of weakly localized perturbations. Constructs phase shifts through resolvent analysis rather than relying on Evans function zeros. Distinguishes between Evans and Fourier eigenmodes when marginal group velocities are directed toward the wave interface.

Result: Proves convergence to shifted wave profiles when initial conditions are close to fast wave profiles in optimal topology. Demonstrates phase shift construction is possible even when translational eigenvalue is not associated with a zero of the Evans function.

Conclusion: The resolvent kernel approach provides a robust framework for analyzing monostable wave stability under optimal conditions, overcoming limitations of traditional Evans function methods and enabling phase shift construction in more general settings.
...
TL;DR: The paper studies a nonlocal geometric energy based on p-oscillation of normal/tangent vectors, proves optimal geometric inequalities with variational constants, and characterizes optimal shapes under constraints.


<details>
  <summary>Details</summary>
Motivation: To investigate geometric energies related to the oscillation of unit normal vectors for hypersurfaces or unit tangent vectors for curves, establishing optimal inequalities and characterizing extremal shapes.

Method: Analyzes a nonlocal energy functional defined by p-oscillation, determines optimal constants through variational problems over probability measures on spheres, and studies existence and characterization of optimal sets under perimeter and volume constraints.

Result: Proves existence of optimal sets for the energy under perimeter and volume constraints, characterizes their shape, and establishes optimal geometric inequalities with constants c(n,p) and C(n,p) determined by variational problems on sphere measures.

Conclusion: The nonlocal p-oscillation energy satisfies sharp geometric inequalities with variational constants, and optimal shapes exist and can be characterized under perimeter/volume constraints, with extremal measures depending critically on p.
...
TL;DR: The paper studies blow-up solutions for weakly dispersive perturbations of inviscid Burgers equation, showing shock formation at a single point with C^{1/3} Hölder regularity and convergence to stable self-similar Burgers solutions.


<details>
  <summary>Details</summary>
Motivation: To understand finite-time blow-up (shock formation) in non-local weakly dispersive perturbations of inviscid Burgers equation, particularly the Fornberg-Whitham equation, by constructing explicit blow-up solutions and analyzing their properties.

Method: Constructs a blow-up solution that displays 'shock-like' singularity (wave breaking) at a single point, analyzes asymptotic convergence in self-similar variables to stable self-similar solutions of inviscid Burgers equation, and establishes Hölder regularity at blowup point.

Result: Successfully constructs blow-up solution with single-point shock formation, proves asymptotic convergence to stable self-similar Burgers solutions, and demonstrates solution possesses C^{1/3} Hölder regularity at blowup point.

Conclusion: The paper provides precise characterization of blow-up behavior in weakly dispersive Burgers perturbations, showing shock formation occurs at isolated points with specific regularity properties and connects to classical Burgers self-similar solutions.
...
TL;DR: The paper studies blow-up behavior and Liouville-type theorems for generalized CH-KP equations with smooth nonlinear term g(u). It establishes blow-up criteria independent of initial data regularity, proves blow-up theorems using characteristic lines and Riccati inequality, and extends results to polynomially controlled g'(u). Also proves a Liouville-type uniqueness theorem.


<details>
  <summary>Details</summary>
Motivation: To understand the blow-up behavior and establish Liouville-type theorems for generalized Camassa-Holm-Kadomtsev-Petviashvili (CH-KP) equations with general nonlinear terms, extending beyond classical cases to more general nonlinearities.

Method: Uses continuation method to establish blow-up criterion independent of initial data regularity. Employs characteristic lines, a priori estimates, and Riccati inequality to prove blow-up theorems. Extends results to polynomially controlled g'(u) cases. Uses analytical techniques for Liouville-type theorem under specific growth conditions on g(u).

Result: 1) Blow-up criterion independent of initial data regularity. 2) Blow-up theorem and weighted blow-up result when g'(u) is uniformly bounded. 3) Extension of blow-up results to polynomially controlled g'(u), covering classical CH-KP nonlinearities. 4) Liouville-type uniqueness theorem under condition g(u) ≥ γu² with u ≠ 0, g(u) > γu².

Conclusion: The paper successfully establishes comprehensive blow-up criteria and Liouville-type theorems for generalized CH-KP equations with various nonlinear terms, providing important analytical tools for understanding solution behavior and uniqueness properties in this class of equations.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the regularity theory for fractional porous medium-type equations, particularly in the fast diffusion regime where solutions can develop singularities. Understanding local boundedness under minimal assumptions is crucial for analyzing solution behavior and developing the theory for these nonlinear nonlocal equations.

Method: The authors likely use techniques from nonlinear potential theory, De Giorgi iteration methods, and fractional calculus. They probably establish local energy estimates and employ Moser iteration adapted to the nonlocal setting, working with optimal tail conditions that are minimal for the fast diffusion case.

Result: The main result proves that solutions to fractional porous medium-type equations in the fast diffusion regime are locally bounded, assuming only optimal tail conditions. This extends regularity theory to the nonlocal setting with minimal assumptions.

Conclusion: The paper successfully establishes local boundedness for solutions to fractional porous medium-type equations in fast diffusion under optimal tail assumptions, advancing the regularity theory for these important nonlinear nonlocal evolution equations.
...
TL;DR: Unique recovery of piecewise constant variable orders in time-fractional diffusion from single boundary measurement, without monotonicity assumptions.


<details>
  <summary>Details</summary>
Motivation: Identify heterogeneous media in anomalous diffusion processes by recovering spatially dependent variable orders from boundary flux measurements.

Method: Combines harmonic function properties, linearization in Laplace domain, and tools from complex, asymptotic, and geometrical analysis.

Result: Establishes new uniqueness results for inverse problem with piecewise constant variable orders, without monotonicity conditions, with weaker regularity assumptions and extension to higher dimensions.

Conclusion: The approach successfully addresses the inverse problem of recovering variable orders in time-fractional diffusion models from single boundary measurements under more general conditions.
...
TL;DR: The paper proposes a minimal mathematical framework for tumor go-or-grow dynamics with two cell populations (migratory vs proliferative) and shows how to reduce it to a single mixed-phenotype equation under cyclic hypoxia.


<details>
  <summary>Details</summary>
Motivation: To develop a minimal mathematical framework that captures the essential dynamics of tumor cell behavior where cells switch between migratory (go) and proliferative (grow) phenotypes based on oxygen availability, and to explore whether this two-population model can be simplified.

Method: Develop a mathematical model with two phenotypically distinct populations: one migrates via linear diffusion, the other proliferates in oxygen-dependent manner. Local oxygen concentration controls phenotype transitions. Then investigate reduction to a single mixed-phenotype equation under cyclic hypoxia conditions, connecting the two-population model to a reduced single-population model with oxygen-dependent diffusion and proliferation in fast-switching regime.

Result: Established a theoretical connection between the minimal go-or-grow model with distinct populations and a reduced model describing a single-cell population with oxygen-dependent diffusion and proliferation. This reduction is valid in the fast-phenotypic-switching regime and was validated through numerical simulations.

Conclusion: The go-or-grow tumor dynamics can be mathematically reduced from a two-population model to a simpler single mixed-phenotype equation under cyclic hypoxia conditions, particularly in fast-switching regimes, providing theoretical validation for simplified modeling approaches.
...
TL;DR: The paper provides mathematical justification for the Rayleigh-Plateau instability phenomenon in capillary water jets, proving existence of invariant manifolds for Eulerian free-boundary systems.


<details>
  <summary>Details</summary>
Motivation: To mathematically justify experimental observations of the Rayleigh-Plateau instability where capillary water jets are exponentially unstable under long-wave perturbations but stable under short-wave perturbations, and to answer Lin-Zeng's question about invariant manifolds for Eulerian free-boundary systems.

Method: Models water jet motion as irrotational Eulerian free-boundary system with surface tension. Uses paradifferential analysis to construct "paradifferential propagator" for linear paradifferential hyperbolic systems along elliptic directions, enabling Lyapunov-Perron type arguments that balance regularity loss in quasilinear problems.

Result: Proves that unstable directions in linearized system (long-wave perturbations) are tangent to unstable invariant manifold of full nonlinear system, while elliptic directions (short-wave perturbations) are tangent to center invariant set in generalized sense.

Conclusion: Provides mathematical justification for Rayleigh-Plateau instability observations and positive answer to Lin-Zeng's question about invariant manifolds. The paradifferential propagator method balances regularity loss and can be generalized to broader class of PDEs.
...
<details>
  <summary>Details</summary>
Motivation: To understand the behavior of incompressible Navier-Stokes equations with initial conditions consisting of polygonal vortex crystals (with/without central vortex), particularly focusing on co-rotating configurations and their evolution before vortex merging occurs.

Method: Exploiting symmetries and stability properties of the system to describe and control solutions. The approach leverages the specific geometric configuration of co-rotating vortex crystals to analyze solution behavior.

Result: Successfully describes and controls the solution up to sub-diffusive time scales, which are time scales occurring before the expected onset of vortex merging in the system.

Conclusion: The symmetries and stability of co-rotating vortex crystal configurations in 2D Navier-Stokes equations allow for controlled analysis of solutions on relevant timescales prior to vortex merging phenomena.
...
TL;DR: Pogorelov-type estimate for k-Hessian equation with new condition on degenerate f


<details>
  <summary>Details</summary>
Motivation: To establish Pogorelov-type estimates for k-Hessian equations when the right-hand side f is degenerate, requiring new analytical conditions

Method: Develops new analytical techniques and conditions for handling degenerate f in k-Hessian equations to prove Pogorelov-type estimates

Result: Establishes Pogorelov-type estimates for k-Hessian equations under the new condition on degenerate f

Conclusion: The new condition on degenerate f enables Pogorelov-type estimates for k-Hessian equations, extending previous results
...
<details>
  <summary>Details</summary>
Motivation: To rigorously analyze the low Mach number limit for bi-fluid compressible Navier-Stokes systems with equal pressure and single velocity, establishing convergence from compressible to incompressible regimes in two-phase flows.

Method: Introduction of suitable modulated quantities and a novel relative entropy functional adapted to the two-phase structure, with key techniques including comparison of L1 and L2 norms of partial densities and a variant of Csiszar-Kullback-Pinsker inequality.

Result: As Mach number tends to zero, partial densities converge to constant states, velocity converges to divergence-free vector field, recovering incompressible non-homogeneous fluid system with nontrivial volume fractions transported by the limit flow.

Conclusion: The paper successfully justifies convergence of weak solutions from compressible to incompressible systems in low Mach number regime for bi-fluid flows, providing rigorous mathematical foundation for this important asymptotic limit.
...
TL;DR: The paper studies conformally flat Lipschitz viscosity solutions to the σₖ-Yamabe equation with smooth hypersurface singularities, proving PDE properties for trace/normal derivatives and showing minimality of hypersurfaces for k=2.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the behavior of solutions to the σₖ-Yamabe equation near singular hypersurfaces, particularly for applications like the σₖ-Loewner-Nirenberg problem on annuli, and to establish regularity properties at singularities.

Method: The authors study conformally flat Lipschitz viscosity solutions in the negative cone. They first prove that under natural regularity assumptions, the trace and normal derivatives along the hypersurface satisfy a specific PDE. For k=2, they additionally show the hypersurface is minimal with respect to the solution and analyze formal expansions near the hypersurface.

Result: 1) Under regularity assumptions, trace and normal derivatives of solutions along hypersurface singularities satisfy a certain PDE. 2) For k=2, the hypersurface is minimal with respect to the Lipschitz solution. 3) The paper addresses questions about formal expansion of solutions near hypersurfaces.

Conclusion: The work establishes important regularity properties for σₖ-Yamabe equation solutions with hypersurface singularities, providing PDE constraints on boundary behavior and minimality results for k=2, with implications for understanding singular solutions in geometric PDEs.
...
Method: Proposes an adaptive patching scheme that exploits block-sparse QTT structures using divide-and-conquer approach, adaptively partitioning tensors into smaller patches with reduced bond dimensions.

Result: Demonstrates substantial improvements for sharply localized functions and shows efficient computation of bubble diagrams and Bethe-Salpeter equations.

Conclusion: The adaptive patching scheme opens the door to practical large-scale QTT-based computations that were previously beyond reach due to prohibitive computational costs.
...
TL;DR: A framework using large language models to guide symbolic regression for discovering physical laws from high-dimensional data, validated on perovskite materials with 10^5 reduction in search space.


<details>
  <summary>Details</summary>
Motivation: Traditional symbolic regression methods often produce complex, unphysical formulas when searching vast spaces of possible forms for discovering interpretable physical laws from high-dimensional data.

Method: Introduces a framework that leverages the embedded scientific knowledge of large language models to guide the search process, enabling efficient identification of physical laws in data.

Result: Reduces effective search space by factor of ~10^5, identifies novel formulas for perovskite properties (bulk modulus, band gap, oxygen evolution reaction activity) that outperform previous formulas in accuracy and simplicity.

Conclusion: The LLM-guided approach successfully mitigates combinatorial explosion in symbolic regression, discovers physically meaningful formulas that provide insights and outperform existing models for perovskite materials.
...
TL;DR: Simplified committor-based enhanced sampling method that eliminates costly coordinate gradients while maintaining robust sampling performance.


<details>
  <summary>Details</summary>
Motivation: Original committor-based enhanced sampling method requires computationally expensive gradients with respect to atomic coordinates, limiting practical application to complex systems.

Method: Propose a simplified learning criterion formulated entirely in descriptor space, bypassing explicit coordinate gradients and providing a relaxed upper bound to the original variational principle.

Result: The simplified approach retains robust sampling performance while significantly reducing computational costs, enabling study of processes that would be unfeasible with the original formulation.

Conclusion: A practical and efficient alternative to the original committor-based enhanced sampling method that maintains sampling effectiveness while dramatically reducing computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in building simulation packages for experiments undergoing continuous development in graduate research, highlighting how one-off approaches led to erroneous results and the need for modular, extensible tools.

Method: The mrfmsim package uses directed acyclic graphs (DAGs) to model experiments and employs a plugin system for adding custom experiments and functionalities. Unlike typical DAG workflow packages, it allows flexible customization of experiments post-definition without rewriting internal models.

Result: The package enabled correct results and significantly accelerated development cycles compared to previous one-off approaches that yielded erroneous results. The modularity, extendibility, and readability of the platform proved effective.

Conclusion: mrfmsim provides a flexible, modular framework for MRFM experiment simulation that addresses the challenges of continuous experimental development in research settings, offering advantages over ad-hoc simulation approaches.
...
TL;DR: Ultracold neutral plasmas enable study of extreme plasma regimes; this work examines electron heating mechanisms in moderately coupled, strongly magnetized plasmas, finding disorder-induced heating persists throughout plasma lifetime and achieving record-low electron temperatures.


<details>
  <summary>Details</summary>
Motivation: To understand electron heating mechanisms in ultracold neutral plasmas under extreme conditions of strong coupling and magnetization, and to determine the minimum achievable electron temperatures that define the maximum coupling strength limits.

Method: Using experimentally informed simulations to examine early-lifetime electron heating, focusing on disorder-induced heating and heating from Rydberg atom formation, while analyzing temperature dependence on magnetization and initial electron energy.

Result: Found that disorder-induced heating significantly influences electron temperature throughout plasma lifetime; achieved record-low electron temperature of 0.52 K (with uncertainty) at electron density of 6.1×10¹² m⁻³, determining maximum coupling strength for experimental conditions.

Conclusion: Ultracold neutral plasmas can achieve extremely low electron temperatures, enabling study of strongly coupled plasma regimes; disorder-induced heating plays a persistent role in electron temperature evolution, and the measured temperatures define practical limits for coupling strength in these systems.
...
TL;DR: The paper investigates nonlinear entropy transfer processes in toroidal ITG and ETG turbulence using gyrokinetic entropy balance relations, revealing different transfer mechanisms between the two turbulence types.


<details>
  <summary>Details</summary>
Motivation: To understand how nonlinear entropy transfer processes regulate turbulent transport in toroidal plasma turbulence, specifically comparing ITG and ETG driven turbulence mechanisms.

Method: Uses gyrokinetic entropy balance relations for zonal and non-zonal modes, introduces spectral analysis of "triad" entropy transfer function to examine nonlinear interactions and their effects on turbulent transport.

Result: Found different entropy transfer patterns: ITG turbulence shows substantial non-zonal to zonal transfer during saturation, then zonal flows mediate transfer between non-zonal modes; ETG turbulence shows dominant transfer among low-wavenumber non-zonal modes throughout both phases.

Conclusion: The study reveals distinct nonlinear entropy transfer mechanisms in ITG vs ETG turbulence, with zonal flows playing a key mediating role in ITG transport regulation but not in ETG turbulence.
...
TL;DR: First gyrokinetic simulations of TEM turbulence in LHD with hydrogen isotopes reveal novel isotope effects: collisional TEM stabilization plus enhanced zonal flows cause transport reduction with opposite mass dependence to gyro-Bohm scaling.


<details>
  <summary>Details</summary>
Motivation: To understand how isotope ion mass affects TEM-driven turbulence and zonal flows in fusion plasmas, particularly in non-axisymmetric magnetic configurations like LHD, which hasn't been studied before with gyrokinetic simulations.

Method: First-ever gyrokinetic simulations of TEM-driven turbulence in 3D LHD plasmas using hydrogen isotope ions and real-mass kinetic electrons, examining both linear and nonlinear effects of isotopes and collisions.

Result: Discovered combined effects: isotope ions collisionality stabilizes TEM, while steady zonal flows increase near marginal stability, leading to significant transport reduction with opposite ion mass dependence compared to conventional gyro-Bohm scaling.

Conclusion: Isotope effects on TEM turbulence and zonal flows are universal across different toroidal plasma systems (tokamaks and stellarators), revealing new physics beyond traditional scaling laws.
...
TL;DR: Gyrokinetic Vlasov simulations reveal ITG/TEM turbulent transport in ITER-like plasma, showing D-T fuel imbalance and identifying steady burning conditions with He-ash exhaust.


<details>
  <summary>Details</summary>
Motivation: To go beyond conventional zero-dimensional power balance analysis and investigate realistic turbulent transport in burning plasmas, including D-T fuel imbalance and He-ash accumulation effects.

Method: Multi-species gyrokinetic Vlasov simulations with D, T, He, and real-mass kinetic electrons including inter-species collisions, examining ITG/TEM turbulent transport in ITER-like plasma.

Result: Significant D-T fuel ion imbalance in turbulent particle flux depending on D-T density ratio and He-ash accumulation; identification of several profile regimes satisfying Reiter's steady burning condition; examination of zonal flow and nonthermal He-ash impacts.

Conclusion: First gyrokinetic-simulation-based demonstration of steady burning conditions with He-ash exhaust and D-T fuel inward pinch, revealing complex turbulent transport dynamics crucial for ITER operation.
...
TL;DR: Self-guided femtosecond laser pulses create plasma filaments for wakefield acceleration, enabling high-repetition-rate, tunable acceleration with improved reliability over conventional methods.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of conventional plasma wakefield acceleration schemes that rely on mechanically confined or preformed plasma channels, which require more energy and have lower reproducibility due to stochastic breakdown processes in discharge plasmas.

Method: Using self-guided femtosecond laser pulses propagating in low-pressure gas to generate plasma filaments through intrinsic non-linear light-matter interaction, then using these filaments for beam-driven wakefield acceleration of electron bunches.

Result: Experimental demonstration of beam-driven wakefield acceleration with accelerating fields exceeding 250 MV/m in laser-generated plasma filaments, with excellent agreement between experimental results and numerical simulations.

Conclusion: Laser-based plasma filamentation offers a promising route toward sustainable, high-repetition-rate plasma-based facilities, bridging laser filamentation physics, advanced plasma photonics, and compact accelerator technologies.
...
TL;DR: The paper investigates non-linear dynamics of zonal flows using gyrofluid modified Hasegawa-Wakatani model, focusing on zonal flow merging and chaotic development patterns.


<details>
  <summary>Details</summary>
Motivation: To understand the non-linear dynamics of zonal flows in plasma physics, particularly the merging phenomena and chaotic development of initial zonal flow patterns, which are important for plasma confinement and turbulence regulation.

Method: Uses gyrofluid modified Hasegawa-Wakatani model, derives conservation equations for zonal flow momentum and energy with consistent finite Larmor radius effects, and performs quantitative analysis through numerical simulations.

Result: Nonlinear local Reynolds stress transfer (rather than hyperviscous dissipation) is identified as the main cause of zonal flow merging; the concept of phase transition in thermodynamical sense is discussed in context of zonal flow transition hysteresis.

Conclusion: The study provides insights into zonal flow merging mechanisms and questions the strict applicability of thermodynamic phase transition concepts to zonal flow transitions, highlighting the role of Reynolds stress in flow dynamics.
...
### [39] [Fluctuating polytropic processes, turbulence, and heating](https://arxiv.org/abs/2602.22272)
*G. Livadiotis,D. J. McComas*

Main category: physics.space-ph

TL;DR: Fluctuating polytropic processes explain turbulent heating in solar wind plasma, showing how random fluctuations cause net heating even in adiabatic systems, with applications to pickup ion energy transfer.


<details>
  <summary>Details</summary>
Motivation: To understand how turbulent heating in solar wind plasma can be thermodynamically described through fluctuating polytropic processes, connecting thermodynamics with turbulence theory.

Method: Derived thermodynamic expressions for fluctuating polytropic processes, analyzed heliospheric radial profiles of polytropic index, temperature, and heating rates, and applied to pickup ion energy transfer.

Result: Polytropic fluctuations cause net heating even in adiabatic processes; solar wind cooling is subadiabatic proportional to fluctuation variance; analytical profiles match turbulent heating; derived PUI heating rates fit observations.

Conclusion: Turbulence heats plasma by fluctuating polytropic processes; fluctuating polytropic thermodynamics successfully describes turbulent heating in solar wind and pickup ion energy transfer.
...
### [40] [TorchLean: Formalizing Neural Networks in Lean](https://arxiv.org/abs/2602.22631)
...
<details>
  <summary>Details</summary>
Motivation: There's a semantic gap between neural networks executed in programming environments and their formal verification artifacts, creating risks where guarantees depend on implicit conventions about operator semantics, tensor layouts, preprocessing, and floating-point behavior.

Method: TorchLean unifies: (1) PyTorch-style verified API with eager/compiled modes lowering to shared op-tagged SSA/DAG IR, (2) explicit Float32 semantics via executable IEEE-754 binary32 kernel and proof-relevant rounding models, (3) verification via IBP and CROWN/LiRPA-style bound propagation with certificate checking.

Result: Validated end-to-end on certified robustness, physics-informed residual bounds for PINNs, Lyapunov-style neural controller verification, and mechanized theoretical results including a universal approximation theorem.

Conclusion: TorchLean demonstrates a semantics-first infrastructure for fully formal, end-to-end verification of learning-enabled systems, treating learned models as first-class mathematical objects with precise shared semantics.
...
### [41] [QuadSync: Quadrifocal Tensor Synchronization via Tucker Decomposition](https://arxiv.org/abs/2602.22639)
*Daniel Miao,Gilad Lerman,Joe Kileel*

Main category: cs.CV

TL;DR: Quadrifocal tensors, previously considered impractical, are made practical through a new synchronization framework using Tucker decomposition, enabling camera recovery from multiple views.


<details>
  <summary>Details</summary>
Motivation: Quadrifocal tensors capture more information than pairwise essential matrices but have been considered impractical. The authors challenge this belief to leverage higher-order information for better multi-view reconstruction.

Method: Develop a block quadrifocal tensor framework with Tucker decomposition (rank 4×4×4×4), create the first quadrifocal synchronization algorithm using Tucker decomposition, ADMM, and iteratively reweighted least squares, and establish relationships between quadrifocal, trifocal, and bifocal tensors for joint synchronization.

Result: Numerical experiments show effectiveness on modern datasets, demonstrating the practical potential of quadrifocal tensors and higher-order information in synchronization tasks.

Conclusion: Quadrifocal tensors are practical and valuable for structure from motion, offering superior information capture compared to pairwise methods, with the proposed framework enabling effective multi-camera recovery.
...
### [42] [Multidimensional Task Learning: A Unified Tensor Framework for Computer Vision Tasks](https://arxiv.org/abs/2602.23217)
...
<details>
  <summary>Details</summary>
Motivation: Current computer vision task formulations are constrained by matrix-based thinking, requiring structural flattening that restricts naturally expressible tasks and prevents unified understanding.

Method: Multidimensional Task Learning (MTL) framework using Generalized Einstein MLPs (GE-MLPs) that operate directly on tensors via Einstein product with tensor-valued parameters, enabling explicit dimensional control without information loss.

Result: Classification, segmentation, and detection are proven to be special cases of MTL differing only in dimensional configuration; the task space is strictly larger than matrix-based formulations, enabling principled spatiotemporal and cross-modal predictions.

Conclusion: MTL provides a mathematical foundation for understanding, comparing, and designing computer vision tasks through tensor algebra, overcoming limitations of conventional matrix-based approaches.
...
### [43] [Learning geometry-dependent lead-field operators for forward ECG modeling](https://arxiv.org/abs/2602.22367)
*Arsenii Dokuchaev,Francesca Bonizzoni,Stefano Pagani,Francesco Regazzoni,Simone Pezzuto*

Main category: cs.LG

TL;DR: A neural surrogate model for forward ECG simulations that combines anatomical shape encoding with lead-field prediction, achieving high accuracy with low data requirements and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Current forward ECG models face three key challenges: 1) difficulty achieving high anatomical fidelity due to limited torso imaging in clinical practice, 2) computational scaling issues with the lead-field method for high-density recordings, and 3) no existing approach that simultaneously achieves high fidelity, low data requirements, and computational efficiency.

Method: A two-component shape-informed surrogate model: 1) a geometry-encoding module that maps anatomical shapes into a low-dimensional latent space, and 2) a geometry-conditioned neural surrogate that predicts lead-field gradients from spatial coordinates, electrode positions, and latent codes. This serves as a drop-in replacement for full-order forward ECG models.

Result: The method achieves high accuracy with mean angular error of 5° for lead-field approximation within the torso, and relative mean squared error <2.5% for ECG simulations. It consistently outperforms the widely used pseudo lead-field approximation while maintaining negligible inference cost.

Conclusion: The proposed framework enables high-fidelity ECG simulations in data-limited clinical settings by using compact latent representations that don't require full torso segmentation, while preserving computational efficiency and accuracy superior to existing approximations.
...
TL;DR: A numerical framework for simulating laser cooling of up to 10^5 ions in Penning traps, enabling optimization of 3D crystal cooling with new pathways and enhanced cooling performance.


<details>
  <summary>Details</summary>
Motivation: Large 3D trapped ion crystals offer improved quantum sensing sensitivity but current numerical techniques for studying their laser cooling are inefficient as crystal size increases.

Method: Developed a powerful numerical framework to simulate laser cooling of up to 10^5 ions in Penning traps, applied to characterize and optimize cooling of ellipsoidal 3D crystals.

Result: Discovered new cooling pathways via axial component addition to E×B modes, achieved greatly enhanced cooling of perpendicular kinetic energy below 1 mK in prolate crystals, and proposed optimal trap/laser parameters.

Conclusion: Demonstrates feasibility of preparing large 3D crystals for high-sensitivity quantum science protocols, motivating their use in future experiments.
...
### [45] [Imprints of primordial magnetic fields on the late-time Universe](https://arxiv.org/abs/2602.23263)
*Jennifer Schober,Molly Abramson,Sayan Mandal,Salome Mtchedlidze,Tina Kahniashvili*

Main category: astro-ph.CO

TL;DR: Primordial magnetic fields can survive nonlinear structure formation, but only on scales where dynamo amplification doesn't dominate over gravitational compression during halo collapse.


<details>
  <summary>Details</summary>
Motivation: To understand which spatial scales of primordial magnetic fields can survive the nonlinear processes of structure formation and gravitational collapse in the Universe.
...
Result: At high Reynolds numbers, turbulence triggers small-scale dynamo that amplifies magnetic energy below Jeans scale, significantly modifying magnetic spectrum. Dynamo dominance depends on competition between dynamo growth time and free-fall time.

Conclusion: Resolving Jeans scale and turbulent inertial range in cosmological MHD simulations is crucial to accurately capture interplay between gravitational compression and dynamo amplification, determining which structures retain memory of primordial fields.
...
### [46] [Calibrations for the Sasaki volume on odd spheres and the no-gap problem](https://arxiv.org/abs/2602.22961)
*Jonas Matuzas*

Main category: math.DG

TL;DR: The paper establishes a universal calibrated lower bound for the Sasaki volume functional on odd-dimensional spheres, shows the infimum equals the calibration value with no Lavrentiev gap, and analyzes rigidity properties of calibrated smooth graphs.


<details>
  <summary>Details</summary>
Motivation: To study the minimization problem for the Sasaki volume functional on unit tangent vector fields of odd-dimensional spheres, establishing optimal lower bounds and understanding the geometric rigidity of minimizing configurations.

Method: Uses the Brito-Chacon-Naveira calibration ω on the unit tangent bundle; analyzes smooth graphs and integral currents; constructs explicit smooth recovery sequences; proves uniform nonvanishing estimates for polar-shell normalization.

Result: Proves universal lower bound Vol^S(V) ≥ c(m;1)vol(S^n) with c(m;1)=4^m/binom(2m,m); shows section-constrained stable mass equals calibration value; demonstrates no smooth unit field can be ω-calibrated everywhere for m≥2; proves infimum equals calibration value with no Lavrentiev gap.

Conclusion: The Sasaki volume functional on odd spheres has a sharp lower bound given by the calibration value, which is attainable in the relaxed setting but not by smooth unit vector fields everywhere, with no Lavrentiev phenomenon occurring.
...
### [47] [Growth-controlled photochromism in yttrium oxyhydride thin films deposited by HiPIMS and pulsed-DC magnetron sputtering](https://arxiv.org/abs/2602.22951)
*M. Zubkins,E. Letko,E. Strods,V. Vibornijs,D. Moldarev,K. Sarakinos,K. Mizohata,K. Kundzins,J. Purans*

Main category: cond-mat.mtrl-sci

TL;DR: Comparison of photochromic YHO thin films deposited by HiPIMS vs pulsed-DCMS shows significant differences in photochromic performance, optical properties, and microstructure despite similar composition.


<details>
  <summary>Details</summary>
Motivation: To investigate how different deposition techniques (HiPIMS vs pulsed-DCMS) affect the photochromic, optical, and structural properties of oxygen-containing yttrium hydride (YHO) thin films.

Method: Deposited YHO thin films using reactive HiPIMS and pulsed-DCMS, analyzed using optical emission spectroscopy to characterize plasma composition, measured critical working pressure requirements, and compared optical properties (transmittance, photochromic contrast, band gap) and structural characteristics (crystallinity, orientation).

Result: HiPIMS requires higher critical pressure (1.0 Pa vs 0.5 Pa), produces films with lower photochromic contrast (9% vs 34%), higher band gap (2.94 eV vs 2.70 eV), and different microstructure (polycrystalline random orientation vs <100> preferred orientation) compared to pulsed-DCMS films.

Conclusion: Beyond chemical composition, thin-film growth conditions and microstructure significantly influence photochromic performance of YHO, with pulsed-DCMS producing superior photochromic properties compared to HiPIMS.
...
### [48] [Fluctuations in the weakly coupled 4D Anderson Hamiltonian](https://arxiv.org/abs/2602.22509)
*Simon Gabriel,Tommaso Rosati*

Main category: math.PR

TL;DR: Proves Gaussian fluctuations of Anderson Hamiltonian in d=4 at weak coupling via Feynman diagrams and BPHZ renormalization, with explicit variance and predicted phase transition at critical coupling.


<details>
  <summary>Details</summary>
Motivation: Study the weak coupling limit of the Anderson Hamiltonian in critical dimension d=4 to understand fluctuations about the Laplacian's Green's function and characterize the limiting distribution.

Method: Combinatorial analysis of Feynman diagrams and detailed study of BPHZ renormalization of the model, characterizing limiting distribution in terms of primitive blow-ups.

Result: Proves Gaussian fluctuations about Green's function with explicit effective variance, shows no Laplacian renormalization present, and predicts phase transition at critical coupling constant.

Conclusion: The approach provides rigorous results for Anderson Hamiltonian in d=4 and appears applicable to a broad class of equations, establishing Gaussian fluctuations in weak coupling regime.
...
### [49] [A Lorentz-Covariant Spectral Universality of Stochastic Fields](https://arxiv.org/abs/2602.23195)
*Alexander G. Tevzadze*

Main category: astro-ph.HE

TL;DR: Lorentz-covariant spectral universality: temporal and spatial power spectra cannot be related by covariant local mapping in >1D; temporal index is symmetry-protected and offset from spatial index by universal geometric factor.


<details>
  <summary>Details</summary>
Motivation: To establish a Lorentz-covariant framework for understanding spectral properties of stationary stochastic fields in Minkowski spacetime, addressing limitations of non-covariant spectral analysis.

Method: Derivation of Lorentz-covariant spectral universality principles using relativistic field theory and symmetry analysis, examining constraints on covariant local mappings between temporal and spatial power spectra.
...
Conclusion: Spectral universality breaks down for anisotropic scaling and dispersion dominated spectra, establishing the necessity of a Lorentz-covariant formulation of relativistic spectral inference for proper analysis of stochastic fields in relativistic contexts.
...
### [50] [HELIOS: A surface integral equation software for light scattering in homogeneous, periodic, and stratified environments](https://arxiv.org/abs/2602.23097)
*Parmenion S. Mavrikakis,Olivier J. F. Martin*

Main category: physics.optics

TL;DR: HELIOS is an open-source SIE software for modeling light scattering by particles in homogeneous/layered media and periodic backgrounds, using PMCHWT formulation with RWG basis functions, Ewald transformation for periodic structures, and efficient layered media Green's tensor computation.


<details>
  <summary>Details</summary>
Motivation: There's a need for reliable computational tools to model light scattering in complex environments including homogeneous media, layered structures, and periodic backgrounds for applications in photonic crystals, metasurfaces, and optical scattering analysis.

Method: Uses PMCHWT surface integral equation formulation with triangular mesh discretization and RWG basis functions. For periodic structures: Ewald transformation to handle 2D lattice infinite series. For layered media: matrix-friendly approach for Green's tensor with Sommerfeld integrals and tabulation-interpolation acceleration. C++ backend with Python interface for workflow management.

Result: HELIOS demonstrates accuracy and versatility through various examples covering all its functionalities, providing a comprehensive tool for optical scattering simulations in complex media.

Conclusion: HELIOS is a versatile, open-source SIE software that successfully addresses light scattering problems in homogeneous media, layered structures, and periodic backgrounds with reliable performance and comprehensive functionality.
...
### [51] [The AI Research Assistant: Promise, Peril, and a Proof of Concept](https://arxiv.org/abs/2602.22842)
*Tan Bui-Thanh*

Main category: cs.AI

TL;DR: AI can meaningfully contribute to creative mathematical research through systematic human-AI collaboration, as demonstrated by discovering novel error representations and bounds for Hermite quadrature rules.


<details>
  <summary>Details</summary>
Motivation: To investigate whether AI can truly contribute to creative mathematical research beyond just automating routine calculations, and to understand the risks and benefits of human-AI collaboration in mathematics.

Method: Conducted a detailed case study using systematic human-AI collaboration with multiple AI assistants to discover novel error representations and bounds for Hermite quadrature rules, documenting the complete research workflow with unusual transparency.

Result: Extended results beyond what manual work achieved, formulating and proving several theorems with AI assistance. AI excelled at algebraic manipulation, systematic proof exploration, literature synthesis, and LaTeX preparation, but required rigorous human verification and mathematical intuition.
...
### [52] [A Multi-Diagnostic Observational Framework for Magnetosonic Solitary Waves During Geomagnetic Storms in Solar Cycles 24 and 25 using Cluster II Mission](https://arxiv.org/abs/2602.22614)
*Murchana Khusroo,Yimnasangla*

Main category: astro-ph.SR

TL;DR: This paper investigates magnetosonic solitons during geomagnetic storms using Cluster II data, finding they occur mainly in early storm phases and may serve as precursors to geomagnetic activity.


<details>
  <summary>Details</summary>
Motivation: While solitons are abundant in space plasmas, their observation during geomagnetic storms remains limited. The study aims to understand soliton behavior during large-scale geomagnetic disturbances driven by solar wind-magnetosphere interactions.

Method: Used high-resolution in-situ magnetic field measurements from Cluster II mission. Developed a comprehensive multi-diagnostic observational framework with state-of-the-art analytical techniques to reliably detect and characterize magnetosonic solitons during Solar Cycles 24 and 25 geomagnetic storms.
...
Conclusion: Magnetosonic solitons observed during geomagnetic storms appear to be early-phase phenomena that could potentially serve as indicators or precursors of upcoming enhanced geomagnetic activity, providing new insights into storm-time dynamics.
...
TL;DR: Light irradiation modifies electronic transport in WSe2 nanoribbons via Floquet engineering, enhancing thermoelectric performance with ZT > 1 across broad temperature range.
...
Method: Combines six-orbital tight-binding with atomic spin-orbit coupling, uses Peierls substitution for periodic driving, maps to effective Floquet Hamiltonian in high-frequency limit, evaluates coherent transport via Landauer-Büttiker formalism, and calculates lattice thermal conductivity from DFT perturbation theory with phonon Boltzmann transport equation.

Result: Light-induced hopping renormalization reshapes band dispersion and transmission near Fermi level, modifying electrical/thermal conductances and Seebeck coefficient. Combined with spin-orbit band splitting and reduced lattice thermal conductivity from enhanced anharmonic scattering, achieves thermoelectric figure of merit ZT > 1 over broad temperature range.

Conclusion: Monochromatic irradiation effectively enhances thermoelectric performance in WSe2 nanoribbons through Floquet engineering of electronic transport and reduction of lattice thermal conductivity, achieving high ZT values suitable for practical thermoelectric applications.
...
TL;DR: QuPDE algorithm automatically finds quadratizations for 1D PDEs, transforming nonquadratic PDEs into quadratic form with minimal auxiliary variables.


<details>
  <summary>Details</summary>
Motivation: Quadratization simplifies analysis, simulation, and control of nonlinear PDEs, but existing methods lack computational tools to automatically find optimal quadratizations.

Method: QuPDE algorithm uses symbolic computation and discrete optimization to find quadratizations for spatially one-dimensional polynomial/rational PDEs.

Result: Successfully applied to 14 diverse PDEs, finding lower-order quadratizations than literature for some cases and discovering new quadratizations for previously untransformed systems.

Conclusion: QuPDE is the first computational tool for PDE quadratization, enabling systematic transformation of nonlinear PDEs into quadratic form with practical applications across multiple fields.
